...
def init_db() -> None:
    ...
    # Non-destructive migrations from previous schema.
...
def stats_for(day: str, date_from: str, date_to: str) -> dict:
    ...
    cur.execute(
        """
        SELECT picker,
               GROUP_CONCAT(DISTINCT truck_number) AS trucks,
               GROUP_CONCAT(DISTINCT work_type) AS actions
        FROM work_logs
        WHERE work_date = ?
        GROUP BY picker
        ORDER BY picker
...
def stats(
    ...
    return templates.TemplateResponse("stats.html", {"request": request, "day": day, "date_from": date_from, "date_to": date_to, **data})
...
def query_logs(work_date: str = "", picker: str = "", work_type: str = "", order_query: str = "") -> list[sqlite3.Row]:
    conn = db()
    sql = "SELECT * FROM work_logs WHERE 1=1"
    args: list[str] = []
    if work_date:
...
def export_excel(rows: list[sqlite3.Row], out: Path) -> None:
    ...
    ws = wb.active
    ws.title = "Журнал"
    ws.append(["Дата", "Сборщик", "Номер заказа", "Тип работы", "Количество", "Комментарий"])
    for r in rows:
        ws.append([r["work_date"], r["picker"], r["order_number"], r["work_type"], r["quantity"], r["comment"]])
    wb.save(out)
...
def home(request: Request) -> HTMLResponse:
    return templates.TemplateResponse(
        "index.html",
        {"request": request, "today": date.today().isoformat(), "pickers": get_pickers(), "work_types": WORK_TYPES},
    )
...
def add_log(
    ...
    conn.execute(
        ...
        VALUES(?, ?, ?, ?, ?, ?, ?)
        """,
        (work_date, picker, order_number, work_type, quantity, comment, datetime.now().isoformat(timespec="seconds")),
    )
    conn.commit()
    conn.close()
    return RedirectResponse("/journal", status_code=303)
...
def journal(
    ...
    rows = query_logs(work_date, picker, work_type, order_query)
    return templates.TemplateResponse(
        "journal.html",
        {
            "request": request,
            ...
            "pickers": get_pickers(),
            "work_types": WORK_TYPES,
            "filters": {"work_date": work_date, "picker": picker, "work_type": work_type, "order_query": order_query},
        },
    )
...
def stats_for(day: str, date_from: str, date_to: str) -> dict:
    ...
    cur.execute(
        """
        SELECT picker, COUNT(DISTINCT order_number) AS orders_count, COALESCE(SUM(quantity), 0) AS total_qty
        FROM work_logs WHERE work_date = ? GROUP BY picker ORDER BY picker
        """,
        (day,),
    )
    ...
    period_rows = cur.fetchall()
    cur.execute("SELECT COALESCE(SUM(quantity), 0) AS total FROM work_logs WHERE work_date BETWEEN ? AND ?", (date_from, date_to))
    total = int(cur.fetchone()["total"])
    cur.execute(
        """
    ...
    types_rows = cur.fetchall()
    conn.close()
    return {"day_rows": day_rows, "period_rows": period_rows, "total": total, "types_rows": types_rows}
...
def stats(
    ...
    return templates.TemplateResponse(
        "stats.html",
        {"request": request, "day": day, "date_from": date_from, "date_to": date_to, **data},
    )
...
def export_journal_xlsx(
    ...
    out = EXPORTS_DIR / f"journal_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    export_excel(query_logs(work_date, picker, work_type, order_query), out)
    return FileResponse(str(out), filename=out.name)
...
def export_stats_pdf(
    ...
    rows = [[r["picker"], str(r["orders_count"]), str(r["total_qty"])] for r in data["period_rows"]]
    out = PRINTS_DIR / f"stats_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
    export_pdf(f"Статистика {date_from} - {date_to}", ["Сборщик", "Заказов", "Объем"], rows, out)
    return FileResponse(str(out), filename=out.name)
